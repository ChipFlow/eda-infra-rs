...
pub fn skip_whitespace_and_comment(mut i: &[u8]) -> IResult<&[u8], ()> {
    ...
    while i.len() > 0 {
        if i[0] == b'/' {
            ...
            i = match value((), tuple((tag("(*"), take_until("*)"), tag("*)"))))(i)
                as IResult<&[u8], ()>
            {
                Ok((i, ())) => i,
                Err(_) => break,
...
/// eats up all spaces.
pub fn ws<'a, F, O>(inner: F) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], O>
where
    ...
    map(
        alt((
            ...
            recognize(pair(
                ...
                take_till(|c| !is_alphanumeric(c) && c != b'_' && c != b'$'),
            )),
        )),
        |s| CompactString::from(u82str_unsafe(s)),
    )(i)
}

fn int(input: &[u8]) -> IResult<&[u8], isize> {
    map(recognize(preceded(opt(char('-')), digit1)), |i| {
        isize::from_str(u82str_unsafe(i)).unwrap()
    })(input)
}

fn uint(input: &[u8]) -> IResult<&[u8], usize> {
    map(recognize(digit1), |i| {
        usize::from_str(u82str_unsafe(i)).unwrap()
    })(input)
}
...
fn literal(i: &[u8]) -> IResult<&[u8], Vec<WirexprBasic>> {
    map(
        tuple((
            ...
            one_of("bBoOdDhH"),
            take_till1(|c| {
                !is_hex_digit(c) && c != b'_' && c != b'x' && c != b'X' && c != b'z' && c != b'Z'
            }),
        )),
        |(width, _, radix, hexint)| {
            ...
            let (mut value, mut is_xz) = if !has_xz {
                ...
                let value =
                    ExtAwi::from_bytes_radix(None, &tmp_value, radix, full_width_nonzero).unwrap();
                let is_xz =
                    ExtAwi::from_bytes_radix(None, &tmp_is_xz, radix, full_width_nonzero).unwrap();
                (value, is_xz)
            };
...
fn assign(i: &[u8]) -> IResult<&[u8], SVerilogAssign> {
    map(
        ...
        |(_, lhs, _, rhs, _)| SVerilogAssign { lhs, rhs },
    )(i)
}
...
fn cell(i: &[u8]) -> IResult<&[u8], SVerilogCell> {
    map(
            ...
            delimited(
                ...
                cut(separated_list0(
                    ...
                    map(
                        ...
                        |(_, name, _, expr, _)| expr.map(|e| (name, e)),
                    ),
                )),
        ...
        |(macro_name, cell_name, ioports)| SVerilogCell {
            macro_name,
            cell_name,
            ioports: ioports.into_iter().filter_map(|port| port).collect(),
        },
    )(i)
}
...
fn sverilog(i: &[u8]) -> IResult<&[u8], SVerilog> {
    map(
        preceded(
            many0_count(ws(char(';'))),
            ws(many0(terminated(ws(module), many0_count(ws(char(';')))))),
        ),
        |modules| SVerilog { modules },
    )(i)
}
    ...
    fn from(e: nom::Err<nom::error::Error<&[u8]>>) -> Self {
        match e {
            ...
            nom::Err::Error(e) | nom::Err::Failure(e) => ParseError {
                code: e.code,
                partial_input: std::str::from_utf8(&e.input[..e.input.len().min(50)])
                    .unwrap()
                    .to_string(),
            },
        }
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "error {:?} at: {}", self.code, self.partial_input)
    }
}
