...
fn parse_insert_fns(source: &str, fns: &mut Vec<FnSig>) -> Option<()> {
    ...
    for node in cpp.children(&mut cpp.walk()) {
        ...
        skip_ifn!(&source[node.child_by_field_name("value")?.byte_range()] == "\"C\"");

        let node = node.child_by_field_name("body")?;
        skip_ifn!(node.kind() == "function_definition");
        skip_ifn!(&source[node.child_by_field_name("type")?.byte_range()] == "void");

        let node = node.child_by_field_name("declarator")?;
        let raw_name = String::from(&source[node.child_by_field_name("declarator")?.byte_range()]);

        let params = node.child_by_field_name("parameters")?;
        let (param_names, param_types) = params
            ...
            .filter_map(|param| {
                ...
                let typ = String::from(&source[param.child_by_field_name("type")?.byte_range()]);
                let isconst = match param.child(0) {
                    Some(c)
                        if c.kind() == "type_qualifier" && &source[c.byte_range()] == "const" =>
                    {
                        true
                    }
                    _ => false,
                ...
                let (name, fnparam) = match decl.kind() {
                    ...
                    "array_declarator" => {
                        let size = ArraySize(String::from(
                            &source[decl.child_by_field_name("size")?.byte_range()],
                        ));
                        let inner_decl = decl.child_by_field_name("declarator")?;
                        skip_ifn!(
                            inner_decl.kind() == "parenthesized_declarator"
...
pub fn bindgen(source_list: impl IntoIterator<Item = impl AsRef<Path>>, dest: impl AsRef<Path>) {
    let root_dir = PathBuf::from(&env::var("CARGO_MANIFEST_DIR").unwrap());

    let mut fns = vec![];
    for source_file in source_list {
        let source = fs::read_to_string(root_dir.join(source_file.as_ref())).unwrap();
    ...
    #[cfg(feature = "ulib")]
    let mut ufuncs: IndexMap<&str, Vec<&FnSig>> = IndexMap::new();
    #[cfg(feature = "ulib")]
    for f in &fns {
    ...
    for (uf, impls) in ufuncs {
        let f0 = &impls[0];
        if impls.len() > 1 && impls[1..].iter().any(|i| i.param_types != f0.param_types) {
