...
pub fn export_csrc() {
    ...
    println!(
        "cargo:ucc_csrc_pkg_name={}",
        env::var("CARGO_PKG_NAME").unwrap()
    );
    // for detecting dependency rebuild.
    println!(
        "cargo:ucc_csrc_build_time={}",
        SystemTime::now()
    ...
    println!(
        "cargo:ucc_csrc_manifest_dir={}",
        env::var("CARGO_MANIFEST_DIR").unwrap()
    );
    println!("cargo:ucc_csrc_out_dir={}", env::var("OUT_DIR").unwrap());
}
...
pub fn import_csrc() -> PathBuf {
    println!("cargo:rerun-if-changed=csrc");
    let out_dir = Path::new(&env::var("OUT_DIR").unwrap()).join("ucc_csrc_includes");
