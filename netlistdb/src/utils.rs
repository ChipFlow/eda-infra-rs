...
pub(crate) fn find_top_module<'i>(
    ...
    if let Some(top) = top {
        if let Some((k, (v1, v2))) = modules.get_key_value(top) {
            ...
            clilog::error!(
                NL_SV_TOPMODULE_NF,
                "Top module {} not found in the verilog code",
                top
            );
            return None;
        }
        ...
        let unrefs: Vec<_> = modules
            .iter()
            .filter(|(s, _)| !referenced.contains(s))
            .collect();
        if unrefs.len() == 1 {
            let (s, (m, mm)) = unrefs[0];
            clilog::info!(NL_SV_GUESSTOP, "The top module is guessed to be {}.", s);
            Some((s, m, mm))
...
pub fn enum_in_width(w: Option<SVerilogRange>) -> impl Iterator<Item = Option<isize>> {
    match w {
...
fn eval_expr_len(def_widths: &HashMap<CompactString, SVerilogRange>, expr: &Wirexpr) -> usize {
    #[inline]
    fn len_basic(
        ...
        use WirexprBasic::*;
        match exprbasic {
            Full(s) => match def_widths.get(s.as_str()) {
...
impl ModuleMap {
    pub fn from(m: &SVerilogModule) -> ModuleMap {
        ...
        let def_widths: HashMap<CompactString, SVerilogRange> = m
            .defs
            .iter()
        ...
        for SVerilogWireDef { name, typ, .. } in &m.defs {
            ...
            match def_types.get_mut(name) {
                Some(v) => match (&v, typ) {
                    (Wire, Input | Output | InOut) => {
                        *v = *typ;
                    }
                    (_, Wire) => {}
                    _ => {
                        assert_eq!(v, typ, "conflicting def");
                    }
                },
                None => {
                    def_types.insert(name.clone(), *typ);
                }
            }
        }
            ...
            .filter_map(|def| {
                ...
                match def {
                    ...
                    Conn(name, expr) => {
                        ...
                        if width > 1 {
                            return Some((name.clone(), SVerilogRange(width as isize - 1, 0)));
                        }
        ...
        fn eval_basic<'a>(
            ...
            match exprbasic {
                ...
                Literal(size, value, is_xz) => Right(Right({
                    ...
                    (0..*size)
                        .rev()
                        .map(move |i| Const((((is_xz >> i & 1) << 1) | (value >> i & 1)) as u8))
                })),
            }
        }
        use Wirexpr::*;
...
pub(crate) fn estimate_size<'i>(
    ...
    if !parent_modules.insert(cur_name) {
        clilog::error!(
            NL_SV_RECUR,
            "module {} has recursion which is NOT allowed",
            cur_name
        );
        return None;
    }
